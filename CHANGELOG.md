...
### 新增 (Added)
- **風格游標 (Custom Cursor)**：
  - **原生整合**：實作 CSS `cursor` 原生替換方案，提供零延遲的圓潤箭頭 (Arrow) 與小手 (Hand) 游標。
  - **主題感知**：游標顏色會隨深淺主題自動切換 (Dark/Light)，確保最佳對比度。
  - **完整支援**：新增文字輸入 (`ibeam`) 與圖片縮放 (`zoom/crosshair`) 的對應樣式。
  - **偏好設定**：於電腦版控制中心新增開關，可隨時啟用/停用。

### 修復 (Fixed)
- **穩定性修復**：
  - **目錄顯示**：修復文章目錄 (TOC) 因載入競態導致有時無法顯示的問題。
  - **渲染效能**：修復 `PostDetail` 因 `ReactMarkdown` 組件未記憶化導致的無限渲染迴圈 (Infinite Loop)。
  - **建置錯誤**：移除 `Essay.jsx` 中引用不存在資源導致的 `esbuild` 錯誤。

## [v1.14.0] - 2026-01-15
### 新增 (Added)
- **行動版體驗升級 (Mobile Experience)**：
  - **底部選單 (Bottom Sheet)**：重構手機版漢堡選單，改為現代化底部抽屜設計，支援果凍 (Jelly) 彈性動畫與交錯淡入效果。
  - **控制中心 (Control Center)**：實作「原點展開」動畫，視窗會從按鈕位置縮放展開；優化手機版面為單一卡片，支援「互動/選單」滑動切換。
- **分類頁面優化 (Categories)**：
  - **三欄佈局**：手機版網格改為緊湊的 3 欄設計，提升資訊展示效率。
  - **流暢互動**：按鈕添加 `transition-all` 過渡，選中效果由粗框改為飽和度增強與內縮，避免溢出與遮擋。
- **評論導航**：實作深度連結 (Deep Linking)，點擊控制中心的「最近評論」可精確跳轉並高亮顯示目標留言。

### 優化 (Changed)
- **視覺拋光 (UI Polish)**：
  - **導覽列**：移除漢堡選單的懸停放大效果，更換控制中心圖示為 `fa-layer-group` (圖層群組)。
  - **統一遮罩**：修正控制中心與選單的背景遮罩透明度不一致問題。
- **介面調整**：優化控制中心的手機版設定按鈕，僅保留圖示以節省空間。
